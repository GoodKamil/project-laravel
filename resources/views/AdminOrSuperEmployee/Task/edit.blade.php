...
@section('content')
    <div class="card">
        <h5 class="card-header">Podgląd zadania</h5>
        <div class="card-body">
            <form action="{{route('AdminOrSuperEmployee.task.update',['idT'=>$task->id_T])}}" method="post">
                @csrf
                <div class="mb-3">
                    ...
                    <select class="form-control  @error('selectUsers') is-invalid @enderror" aria-label="lista pracowników" id="selectUser"  name="selectUser">
                        @foreach($users as $row)
                            <option @selected(old('selectUser',$task->id_U)===$row->id_U)  value="{{$row->id_U}}">{{$row->first_name .' '. $row->last_name}}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" class="form-control  @error('description') is-invalid @enderror" id="description" >{{old('description',$task->Description)}}</textarea>
                ...
                <div class="row mb-3">
                    <div class="col">
                        ...
                        <input type="datetime-local" class="form-control @error('dateOd') is-invalid @enderror"  aria-label="Termin od" name="dateOd" id="dateOd" value="{{old('dateOd',$task->DateFrom)}}">
                        @error('dateOd') <div class="invalid-feedback">{{ $errors->get('dateOd')[0]}}</div>@enderror
                    </div>
                    <div class="col">
                        <label for="dateDo" class="form-label">Data do:</label>
                        <input type="datetime-local" class="form-control @error('dateDo') is-invalid @enderror"  aria-label="Termin do" name="dateDo" id="dateDo" value="{{old('dateDo',$task->DateTo)}}">
                        @error('dateDo') <div class="invalid-feedback">{{ $errors->get('dateDo')[0]}}</div>@enderror
                    </div>
                </div>
                <div class="form-check mb-4">
                    <input class="form-check-input" @checked($task->priority===1) type="checkbox" value="1" id="priority" name='priority'>
                    <label class="form-check-label" for="priority">Priorytet</label>
                </div>
                <div class="mb-3">
                    <button type="submit" class="btn btn-primary mr-3">Edytuj</button>
                    <button type="submit"  formaction="{{ route('AdminOrSuperEmployee.task.index') }}" formmethod="GET" class="btn btn-light">Powrót</button>
                </div>
            </form>
